...
class TransaksiModel extends Model
{
    ...
    public static function getAllMinMaxTranTime($dateStart = '', $dateEnd = '', $data = null)
    {
        ...
        if(!empty($data)) {
            $res = TransaksiModel::join('employee', 'tran_cardNo', '=', 'emp_code')->selectRaw("emp_id, tran_cardNo, MIN(tran_time) as wkt_min, MAX(tran_time) as wkt_max, time(MIN(tran_time)) as time_min, time(MAX(tran_time)) as time_max, date(tran_time) as tgl")
    ...
    public static function getAllEmp($dateStart = '', $dateEnd = '')
    {

        $dateStart = !empty($dateStart) ? $dateStart : date('Y-m-01');
        $dateEnd = !empty($dateEnd) ? $dateEnd : date('Y-m-t');

        $res = TransaksiModel::selectRaw("tran_cardNo")
            ->whereRaw('date(tran_time) BETWEEN "' . $dateStart . '" AND "' . $dateEnd . '"')
            ->groupBy('tran_cardNo')
            ->get();
        return $res;
