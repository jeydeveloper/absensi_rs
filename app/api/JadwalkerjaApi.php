...
class JadwalkerjaApi extends \App\Api\BaseApi
{
        ...
        if (!empty($result)) {
            ...
            foreach ($result as $key => $value) {
                ...
                for ($i = $len; $i <= $forLimit; $i++) {
                    ...
                    if ($absenceLabel == "OFF") {
                        $btnCss = 'class="btn btn-block btn-sm btn-danger" style="width:80px;position:relative;color:#ffffff;"';
                        $onClick = 'onclick="doAlert(\'' . $generateId . '\', \'' . $value->emp_id . '\', \'' . $lblShift . '\', \'' . $scheduleDate . '\', \'' . $absenceLabel . '\', \'\', \'\')"';
                    } elseif ($absenceLabel == "ALPHA") {
                        $btnCss = 'class="btn btn-block btn-sm" style="width:80px;position:relative;background-color:' . $dataEmpHasSchedule[$value->emp_id][$generateId]['color'] . ' !important;color:#ffffff;"';
                        $onClick = 'onclick="doAlert(\'' . $generateId . '\', \'' . $value->emp_id . '\', \'' . $lblShift . '\', \'' . $scheduleDate . '\', \'' . $absenceLabel . '\', \'\', \'\')"';
                        ...
                        $btnCss = 'class="btn btn-block btn-sm btn-default" style="width:80px;position:relative;color:#ffffff;"';
                    ...
                    if (!in_array($dayNo, [6, 0])) {
                        ...
                        if (!empty($dataEmpAbsence[$value->emp_code][$scheduleDate]['time_min']) AND !empty($dataEmpAbsence[$value->emp_code][$scheduleDate]['time_max']) AND $dataEmpAbsence[$value->emp_code][$scheduleDate]['time_min'] == $dataEmpAbsence[$value->emp_code][$scheduleDate]['time_max']) {
                            ...
                            if ($intMinAbsence <= $batasAbsenMasuk) {
                                ...
                                if ($hasVerticalSpan) $spanVertical = '<span style="padding: 2px;background-color:#e74c3c;color:#fff;position: absolute;right: 0;top: 0;bottom: 0;">&nbsp;</span>';
                            } else {
                                $tooltip = $this->explodeLabelAbsen($tooltip, 1);
                                if ($hasVerticalSpan) $spanVertical = '<span style="padding: 2px;background-color:#e74c3c;color:#fff;position: absolute;left: 0;top: 0;bottom: 0;">&nbsp;</span>';
                            }
                        }
                    }
    ...
    private function getButtonJadwal($arrParam = null)
    {
        ...
        if ($absenceLabel == "OFF") {
            ...
            $btnCss = 'class="btn btn-block btn-sm" style="width:80px;position:relative;background-color:' . $dataEmpHasSchedule[$arrParam['empId']][$generateId]['color'] . ' !important;color:#ffffff;"';
            $onClick = 'onclick="doAlert(\'' . $generateId . '\', \'' . $arrParam['empId'] . '\', \'' . $lblShift . '\', \'' . $scheduleDate . '\', \'' . $absenceLabel . '\', \'\', \'\')"';
